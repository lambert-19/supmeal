...
- Historique Git : le mot de passe Postgres qui y était exposé a été changé (voir `SUIVI_PROJET.md`, section malus)  reste à décider si l'historique doit malgré tout être réécrit avant toute publication du dépôt.
...
- Messages vocaux dans le chat de cookbook : évoqué puis volontairement écarté (plus lourd  enregistrement audio navigateur, upload, lecteur dédié).
