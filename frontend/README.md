...
src/
  ...
  lib/
    ...
    socket.js       instance socket.io-client unique (getSocket()), connectée/déconnectée par auth-store.js
    stores/presence-store.js  présence en ligne { [userId]: { online, lastSeenAt } }, alimenté par use-cookbook-presence.js
...
- **Exporter** : un `Sheet` affiche d'abord l'avertissement "données en clair" (non chiffrées), puis une liste à cocher des recettes (toutes sélectionnées par défaut, bouton "Tout sélectionner/désélectionner", compteur "x/y") pour choisir précisément lesquelles exporter, avant de proposer trois formats  **JSON** (natif, uniquement les champs portables d'une recette : titre, ingrédients, étapes, temps, portions, tags, images, source), **CSV** (`papaparse`, ingrédients/étapes aplatis dans une cellule texte lisible) et **Mealie (JSON)** (approximation du schéma d'export du logiciel [Mealie](https://mealie.io/) : `recipeIngredient`, `recipeInstructions`, durées ISO 8601 `PTnM`, `orgURL` ; contrairement au vrai Mealie qui ne gère qu'une image principale, `images` exporte le tableau complet). Téléchargement via `file-saver` ; export refusé avec un message si aucune recette n'est cochée.
...
- **Chat de groupe par cookbook** : onglet "Discussion" de `cookbooks/cookbook-detail-page.jsx` (`components/cookbooks/cookbook-chat.jsx`), visible par tous les membres mais l'envoi de message est réservé aux rôles créateur/éditeur/commentateur (`canComment`, voir plus haut)  un lecteur voit l'historique et l'état vide adapté, sans formulaire de saisie, **et le serveur refuse aussi la requête (403)** si le formulaire était contourné. Bulles alignées à droite pour ses propres messages, à gauche pour les autres. Branché sur `GET/POST /cookbooks/:id/messages` (`lib/stores/messages-store.js`) **et désormais en temps réel via Socket.io** (`lib/socket.js`, `hooks/use-cookbook-messages.js`) : plus besoin de recharger la page pour voir un message posté par un autre membre  voir `backend/README.md` pour le détail de l'authentification/des rooms côté serveur.
  - **Emojis** : sélecteur (`emoji-picker-react`, thème clair/sombre synchronisé via `next-themes`) inséré dans un panneau positionné au-dessus du champ de saisie, fermé au clic en dehors (écouteur `mousedown` sur `document`, comparé aux refs du panneau et du bouton déclencheur).
  - **Stickers/images** : bouton dédié ouvrant un `<input type="file">` caché  l'image est uploadée immédiatement via `POST /uploads/images` (même pipeline que les photos de recette, 2 Mo max) puis envoyée comme message à part entière (`addMessage(cookbookId, { imageUrl })`, sans texte). Un message peut donc désormais être composé de texte, d'une image, ou des deux (le rendu affiche l'image au-dessus du texte s'il y en a).
...
`src/lib/stores/recipes-store.js` est branché sur `GET/POST /recipes`, `GET/PATCH/DELETE /recipes/:id`, `PATCH /recipes/:id/favorite` (`fetchAll`, `addRecipe`, `updateRecipe`, `deleteRecipe`, `toggleFavorite`, toutes `async`). `hooks/use-my-recipes.js` déclenche `fetchAll()` une fois par session ; les pages détail/édition (`recipe-detail-page.jsx`, `edit-recipe-page.jsx`) font leur propre fetch unitaire (`GET /recipes/:id`) plutôt que de dériver du store liste  nécessaire pour un rafraîchissement de page fiable (voir plus bas). Chaque recette peut contenir jusqu'à 10 images (2 Mo max chacune, jpeg/png/webp/gif) : `components/recipes/image-gallery-input.jsx` les envoie immédiatement à `POST /uploads/images` (`multipart/form-data`) dès qu'elles sont déposées/sélectionnées (spinner pendant l'envoi), et stocke les URLs absolues renvoyées par le serveur dans `images` (le champ react-hook-form reste un simple `string[]`, aucun changement côté `recipe-form.jsx`/schéma zod)  plus de data URL base64 générée côté client. Affichées dans un format 4:3 uniforme (`object-cover`) partout dans l'app.
...
- **OAuth2 réel (Google/GitHub)** : `login-page.jsx` a deux boutons qui ne sont plus des toasts "bientôt disponible" mais de vrais `<a href>` vers `${VITE_API_URL}/auth/oauth/google|github` (`lib/api.js#oauthStartUrl`) — navigation plein-page volontaire (pas un appel axios) puisque le flux redirige vers l'écran de consentement du fournisseur puis revient poser le cookie de session côté serveur. Un 401/redirection intermédiaire n'a donc jamais de sens à intercepter côté client ; les erreurs reviennent en query param (`?oauthError=...`) lu une seule fois au montage de `login-page.jsx` (toast, puis nettoyage de l'URL). Voir aussi la section Connexions ci-dessous pour le rattachement depuis Paramètres.
...
- **Champs mot de passe** : `components/password-input.jsx` (`PasswordInput`) ajoute un bouton œil (`Eye`/`EyeOff`, `tabIndex={-1}` pour ne pas casser la tabulation) qui bascule `type="password"`/`type="text"`  utilisé sur tous les champs mot de passe de l'app (connexion, inscription, réinitialisation, changement de mot de passe). Les règles de complexité (8 caractères, 1 majuscule, 1 minuscule, 1 chiffre, 1 caractère spécial  miroir exact de `backend/utils/passwordPolicy.js`) vivent dans `lib/password-strength.js`, réutilisées à la fois par les schémas zod (`schemas/auth.js`, `schemas/settings.js`, via `.refine(meetsPasswordRequirements, ...)`) et par `components/password-strength-meter.jsx` (`PasswordStrengthMeter`), qui affiche en direct sur les champs "nouveau mot de passe" (inscription, réinitialisation, changement de mot de passe) une checklist des exigences (coche verte dès qu'une règle est remplie) ainsi qu'une barre de force faible/moyen/fort (fort = toutes les règles remplies + longueur ≥ 15).
- **Paramètres > Connexions** (`pages/settings/connections-tab.jsx`) : les boutons "Lier" sont de vrais `<a href>` vers `${VITE_API_URL}/auth/oauth/:provider/link` (nécessite une session active, donc réservé à un utilisateur déjà connecté) ; le retour du fournisseur redirige vers `/settings?tab=connections&linked=google` (ou `oauthError=...`), lu au montage via `useSearchParams` pour afficher un toast puis nettoyer l'URL (`settings-page.jsx` lit aussi `?tab=` pour ouvrir directement le bon onglet). "Délier" (`DELETE /users/me/oauth/:provider`, `auth-store.js#disconnectOAuthProvider`) est désactivé avec une explication si c'est le seul moyen de connexion du compte (`user.hasPassword === false` et un seul fournisseur lié) — évite de se retrouver verrouillé hors de son propre compte.
